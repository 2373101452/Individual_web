...
function initGomokuGame(){
  ...
  for(var i=0;i<gomokuSize;i++){
    gomokuBoard[i]=[];
    for(var j=0;j<gomokuSize;j++){ gomokuBoard[i][j]=null; }
  }

  var container = document.getElementById('game-container');
  var boardHtml = '<table class="gomoku-board"><tbody>';
  for(var r=0;r<gomokuSize;r++){
    boardHtml += '<tr>';
    for(var c=0;c<gomokuSize;c++){
      boardHtml += '<td class="gomoku-cell" data-r="'+r+'" data-c="'+c+'" onclick="gomokuClick('+r+','+c+')"></td>';
    }
    boardHtml += '</tr>';
  }
  ...
  container.innerHTML = [
    '<div class="game-wrap game-gomoku">',
      '<div class="game-header">',
        '<h2 class="game-title">五子棋</h2>',
        '<div class="gomoku-info">',
          '<span class="gomoku-turn" id="gomokuTurn"><span class="gomoku-stone black"></span> 黑棋落子</span>',
        '</div>',
      '</div>',
      ...
      '<div class="gomoku-actions">',
        '<div class="gomoku-result" id="gomokuResult"></div>',
        '<button class="game-btn btn-submit" onclick="gomokuReset()">重新开始</button>',
      '</div>',
    '</div>'
  ].join('');
}
...
function gomokuClick(r,c){
  ...
  if(gomokuBoard[r][c] !== null) return;

  gomokuBoard[r][c] = gomokuTurn;
  var cell = document.querySelector('.gomoku-cell[data-r="'+r+'"][data-c="'+c+'"]');
  cell.classList.add(gomokuTurn);
  cell.textContent = '●';

  var win = gomokuCheckWin(r,c,gomokuTurn);
  if(win){
    gomokuOver = true;
    // highlight winning line
    win.forEach(function(pos){
      var wcell = document.querySelector('.gomoku-cell[data-r="'+pos.r+'"][data-c="'+pos.c+'"]');
      wcell.classList.add('win');
    });
    var winnerName = gomokuTurn==='black' ? '黑棋' : '白棋';
    document.getElementById('gomokuResult').textContent = winnerName + '获胜！';
    document.getElementById('gomokuTurn').innerHTML = '<span class="gomoku-stone '+gomokuTurn+'"></span> 游戏结束';
    return;
  }
  ...
  for(var i=0;i<gomokuSize;i++){
    for(var j=0;j<gomokuSize;j++){
      if(gomokuBoard[i][j]===null){ draw=false; break; }
    }
    if(!draw) break;
  }
  if(draw){
    gomokuOver = true;
    document.getElementById('gomokuResult').textContent = '平局！';
    document.getElementById('gomokuTurn').innerHTML = '游戏结束';
    return;
  }

  gomokuTurn = gomokuTurn==='black' ? 'white' : 'black';
  var nextName = gomokuTurn==='black' ? '黑棋落子' : '白棋落子';
  document.getElementById('gomokuTurn').innerHTML = '<span class="gomoku-stone '+gomokuTurn+'"></span> '+nextName;
}
...
function gomokuCheckWin(r,c,color){
  ...
  for(var d=0;d<dirs.length;d++){
    ...
    for(var i=1;i<5;i++){
      ...
      if(nr>=0&&nr<gomokuSize&&nc>=0&&nc<gomokuSize&&gomokuBoard[nr][nc]===color){
        line.push({r:nr,c:nc});
      } else { break; }
    }
    // backward
